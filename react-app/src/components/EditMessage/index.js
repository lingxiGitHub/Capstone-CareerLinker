...
export default function EditMessageComponent({ message, conversationId }) {
  ...
  const handleUpdate = async (e) => {
    ...
    if (message_content.length > 1000) {
      setErrors(["Message must be less than 1000 characters"]);
      return;
    }

    const data = await dispatch(
      updateMessageThunk(updatedMessage, conversationId)
    );
  ...
  return (
    <>
      ...
      <form className="update-message-form" onSubmit={handleUpdate}>
        <ul>
        ...
        <button className="update-message-button" type="submit">
          Update
        </button>
      </form>
    </>
